...
# Set page config
st.set_page_config(
    page_title="Smart Farming Advanced Crop Recommender",
    page_icon="🌱",
    layout="wide"
)
...
def main():
    # Main app header
    st.title("🌱 Smart Farming Advanced Crop Recommender")
    st.markdown("Enter soil and climate data to get advanced crop recommendations based on our combined model")
    ...
    # Load label encoder
    label_encoder_path = "models/combined_label_encoder.pkl"
    ...
    if submit_button:
        ...
        # Display confidence for the top prediction
        confidence = top_probs[0]
        st.write(f"**Confidence:** {confidence:.2%}")
        st.progress(confidence)
        
        # Display top 5 recommendations with probabilities
        st.subheader("Top 5 Recommendations")
        
        # Create a bar chart for top crops
        fig, ax = plt.subplots(figsize=(10, 6))
        colors = ['#2ecc71', '#3498db', '#9b59b6', '#f1c40f', '#95a5a6']  # Different colors for each bar
        ax.barh(top_crops, top_probs * 100, color=colors)
        ...
        for i, prob in enumerate(top_probs):
            ax.text(prob * 100 + 1, i, f'{prob * 100:.1f}%', va='center')
            
        st.pyplot(fig)
        
        # Create two columns for additional information
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            soil_quality = "Good"
            if ph < 5.5 or ph > 7.5:
                soil_quality = "Needs Attention"
            
            # Evaluate climate conditions
            climate_suitability = "Favorable"
            if temperature < 15 or temperature > 35:
                climate_suitability = "Challenging"
            
            # Display assessment
            st.markdown(f"**Soil Quality:** {soil_quality}")
            st.markdown(f"**Climate Suitability:** {climate_suitability}")
            
            # Display soil nutrient levels
            ...
            # Create nutrient level chart
            nutrient_df = pd.DataFrame(nutrient_data)
            fig, ax = plt.subplots(figsize=(8, 4))
            sns.barplot(x='Value', y='Nutrient', data=nutrient_df, ax=ax)
        ...
        with col2:
            ...
            crop_info = {
                ...
                "barley": {
                    ...
                    "growing_conditions": "Cool season crop that grows best in temperatures between 15-25°C.",
                    "soil_requirements": "Prefers well-drained, fertile soils with pH 6.0-7.0.",
                    "water_needs": "Moderate water requirements, more drought-tolerant than wheat."
                },
                "teff": {
                    "description": "Teff is a fine grain native to Ethiopia, used to make injera bread and as a nutritious alternative grain.",
                    "growing_conditions": "Warm season crop that grows well in temperatures between 18-27°C.",
                    "soil_requirements": "Adaptable to various soil types, but prefers well-drained soils with pH 5.5-7.5.",
                    "water_needs": "Moderate water requirements, relatively drought tolerant."
                }
            }
